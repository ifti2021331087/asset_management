...
import { uploadAssetAction } from "@/src/actions/dashboard-actions";
...
interface CloudinaryUploadResponse {
  secure_url: string;
  // You can add other properties here later if you need them, like:
  // public_id: string;
  // format: string;
}
...
function UploadAsset({ categories }: UploadDialogProps) {
  ...
  const handleAssetUpload = async (event: React.FormEvent) => {
    ...
    try {
      ...
      xhr.open(
        "POST",
        `https://api.cloudinary.com/v1_1/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUDE_NAME}/auto/upload`
      );
      ...
      const cloudinaryPromise = new Promise<CloudinaryUploadResponse>((resolve, reject) => {
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            console.error("CLOUDINARY REJECTED:", xhr.responseText);
